...
namespace Server.Engines.NewMagincia
{
    public class WritOfLease : Item
    {
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            list.Add(1150546, ServerList.ServerName); // Shard: ~1_SHARDNAME~
        ...
        private class WritNoteGump : Gump
        {
                ...
                : base(100, 100)
            {
                ...
                if (lease.Expired)
                {
                    args = string.Format("{0}\t{1}\t{2}\t{3}\t{4}", lease.Identifier, lease.Facet, ServerList.ServerName, "", string.Format("{0} {1}", lease.RecallLoc.X, lease.RecallLoc.Y));
                ...
                {
                    args = string.Format("{0}\t{1}\t{2}\t{3}\t{4}", lease.Identifier, lease.Facet, ServerList.ServerName, MaginciaLottoSystem.WritExpirePeriod.ToString(), string.Format("{0} {1}", lease.RecallLoc.X, lease.RecallLoc.Y));
